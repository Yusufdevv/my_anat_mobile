...
class MapOrganizationBloc extends Bloc<MapEvent, MapOrganizationState> {
  ...
  FutureOr<void> _getMoreHospitals(MapGetMoreHospitalsEvent event,
      Emitter<MapOrganizationState> emit) async {
    if (state.hospitalsNext == null) return;
    final result =
        await getHospitalsUseCase(MapV2Params(next: state.hospitalsNext));
    if (result.isRight) {
      emit(state.copyWith(
  ...
  FutureOr<void> _changeTab(
    ...
    if (state.screenStatus.isList) {
      ...
      if (event.haveToLoading) {
        emit(state.copyWith(
            tabChangingStatus: FormzStatus.submissionInProgress));
  ...
  FutureOr<void> _getMoreDoctors(
      MapGetMoreDoctorsEvent event, Emitter<MapOrganizationState> emit) async {
    final result = await getDoctors(
      MapV2Params(next: state.doctorsNext),
    );
    if (result.isRight) {
      emit(
  ...
  FutureOr<void> _onMapCreated(
    ...
    emit(
      state.copyWith(
        ...
        hospitalsNext:
            hospitalResult.isRight ? hospitalResult.right.next ?? '' : null,
        zoomLevel: 15,
        currentLat: lat,
        currentLong: long,
