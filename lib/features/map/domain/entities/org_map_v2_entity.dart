...
extension OrgMapV2EntityExtention on OrgMapV2Entity {
  List<ServiceOrSpecializationEntity> getServiceOrSpecialization(
      {required String? pattern, bool unUsed = false}) {
    if (pattern != null && pattern.isEmpty) return [];
    final v = [
      ...specialization
          .map((e) => ServiceOrSpecializationEntity(
              isService: false, id: e.id, title: e.title))
          .toList(),
      ...service
          .map((e) => ServiceOrSpecializationEntity(
              isService: true, id: e.id, title: e.name))
          .toList()
    ];
