    ...
    return BlocProvider.value(
      ...
      child: GestureDetector(
        ...
        child: Scaffold(
          ...
          body: Column(
            children: [
              BlocSelector<ServicesBloc, ServicesState, int>(
                  selector: (state) => state.serviceCount,
                                        ...
                                        child: TextFormField(
                                          ...
                                          onChanged: (text) {
                                            ...
                                            if (text.trim().length > 1) {
                                              widget.servicesBloc
                                                  .add(ServicesEvent.searchServicesOrg(query: text.trim()));
                                            }

                                            if (text.isEmpty) {
                                              widget.servicesBloc.add(ServicesEvent.getServicesOrg(
              ...
              BlocBuilder<ServicesBloc, ServicesState>(
                builder: (context, state) {
                  if (state.statusOrg.isSubmissionInProgress) {
                    return const Center(child: CupertinoActivityIndicator(color: white));
                  }
                  if (state.statusOrg.isSubmissionSuccess) {
                    return state.servicesOrg.isNotEmpty
                                ...
                                Container(
                                  ...
                                  child: Column(
                                    children: List.generate(
                                      ...
                                      (index) => ServiceItem(
                                        onTap: () {
                                          widget.servicesBloc.add(
                                              ServicesEvent.getSingleService(serviceId: state.servicesOrg[index].id));
                                          Navigator.of(context).push(
                                            fade(
                                                page: ServiceSingleScreen(
                                                    serviceUrl: state.servicesOrg[index].url,
                                                    servicesBloc: widget.servicesBloc)),
                                          );
                                        },
                                        hightlightedText: _controller.text.trim(),
          ...
          bottomNavigationBar: BlocBuilder<ServicesBloc, ServicesState>(builder: (context, state) {
            if (state.statusOrg.isSubmissionSuccess) {
              return state.fetchMoreOrg
                  ? WButton(
                      onTap: () {
