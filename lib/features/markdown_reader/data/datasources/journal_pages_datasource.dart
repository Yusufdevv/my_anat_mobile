...
abstract class JournalPagesDatasource {
  Future<GenericPagination<JournalPageModel>> getPages(
      {required String slug, String? next});

  Future<GenericPagination<JournalOutlineModel>> getPagesTableOfContents(
      {required String slug, String? next});
}
  ...
  Future<GenericPagination<JournalPageModel>> getPages(
    ...
    try {
      ...
      if (response.statusCode != null &&
        ...
        return GenericPagination<JournalPageModel>.fromJson(response.data,
            (p0) => JournalPageModel.fromJson(p0 as Map<String, dynamic>));
      } else {
  ...
  Future<GenericPagination<JournalOutlineModel>> getPagesTableOfContents(
    ...
    try {
      ...
      if (response.statusCode != null &&
          response.statusCode! >= 200 &&
          response.statusCode! < 300) {
        return GenericPagination<JournalOutlineModel>.fromJson(response.data,
            (p0) => JournalOutlineModel.fromJson(p0 as Map<String, dynamic>));
      } else {
        throw ServerException(
            statusCode: response.statusCode!,
            errorMessage: response.data.toString());
